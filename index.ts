 ...
 * 格式化并规范化输入值
 */
const sanitizeValue = (val: string, binding: OnlyNumberOptions, isBlur = false): string => {
  if (val === '') {
    return ''
  }
  ...
  // 1. 根据是否允许负数过滤字符
  let sanitized = cleanedVal.replace(isPositive ? /[^\d.]/g : /[^\d.-]/g, '')
...
const setVal = (el: Element, value: string, vnode: any) => {
  ...
  const emitValue = binding.thousands ? value.replace(/,/g, '') : value
  ...
  if (vnode.component?.emit) {
    vnode.component.emit('update:modelValue', emitValue)
  }
}
...
const createOnlyNumber = (el: Element, bindingValue: OnlyNumberOptions, vnode: any) => {
  ...
  const handleKeypress = (e: KeyboardEvent) => {
    ...
    // 1. 过滤允许的字符
    const re = isPositive ? (precision > 0 ? /\d|\./ : /\d/) : precision > 0 ? /\d|\.|-/ : /\d|-/
  ...
  const handleBlur = () => {
    ...
    const sanitized = sanitizeValue(curValue, binding, true)
    if (curValue !== sanitized) {
      setVal(el, sanitized, vnode)
    }
  }
  ...
  const handleFocus = () => {
    ...
    if (binding.thousands) {
      const curValue = input.value
      const cleaned = curValue.replace(/,/g, '')
      if (curValue !== cleaned) {
        setVal(el, cleaned, vnode)
      }
    }
  }
  ...
  // 初始化格式化值（若当前未处于聚焦状态，则以失焦状态格式化以正确显示千分位等）
  const initVal = input.value
  if (initVal !== '') {
    const isFocused = document.activeElement === input
    const sanitized = sanitizeValue(initVal, bindingValue, !isFocused)
    if (initVal !== sanitized) {
      setVal(el, sanitized, vnode)
    }
  }
}
...
export const onlyNumberDirective: Directive = {
  ...
  updated(el, binding, vnode) {
    ...
    // 当外部绑定的数值改变且未聚焦时，重新进行清洗和千分位格式化
    const input = (
      el.tagName === 'INPUT' ? el : el.querySelector('input')
    ) as HTMLInputElement | null
    if (input && document.activeElement !== input) {
      const curValue = input.value
      const sanitized = sanitizeValue(curValue, bindingValue, true)
      if (curValue !== sanitized) {
        setVal(el, sanitized, vnode)
      }
    }
